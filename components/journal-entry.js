...
class JournalEntry extends HTMLElement {
  ...
  set entry(entry) {
    ...
    if (entry.image) {
      ...
      this.shadowRoot.querySelector('.entry').insertAdjacentHTML('beforeend',entryImage.outerHTML);
      // CODE GOES HERE ^^^
    ...
    if (entry.audio) {
      ...
      this.shadowRoot.querySelector('.entry').insertAdjacentHTML('beforeend',entryAudio.outerHTML);
      // CODE GOES HERE ^^^
